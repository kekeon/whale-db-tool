...
export function formatUpdate(
  ...
  let sqlList: string[] = newDataList.map((item) => {
    ...
    return `UPDATE  \`${db}\`.\`${table}\` SET ${arr.join(
      ","
    )} WHERE ${c} LIMIT 1;`;
  });
...
export function formatUpdateValid(
  ...
  for (let i = 0; i < newDataList.length; i++) {
    ...
    for (let index = 0; index < tableDesc.length; index++) {
      ...
      updateValueList.push(
        "`" +
          fieldName +
          "`=" +
          `${undefinedToValue(newValue[fieldName], tableDesc[index])}`
      );
    }
    ...
    const updateSql = `UPDATE  \`${db}\`.\`${table}\` SET ${updateValueList.join(
      ","
    )} WHERE ${c} LIMIT 1;`;
